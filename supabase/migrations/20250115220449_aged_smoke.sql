-- Drop and recreate backup functions with proper storage handling
...
BEGIN
    ...
    VALUES (
        ...
        'database/' || filename,
        auth.uid(),
        jsonb_build_object('type', 'database_backup'),
        backup_data::text::bytea
    );

    -- Return the filename for the client to handle download
